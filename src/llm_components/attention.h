...
public:
  CausalAttention(int n_heads, int n_embed, float dropout, nn::Pcg32& rng)
    : n_heads_(n_heads),
      n_embed_(n_embed),
      dropout_(dropout),
  ...
  nn::Tensor forward(const nn::Tensor& x) override {
    ...
    q = q.reshape_view({B, T, H, D}).transpose_view(1, 2); // [B, H, T, D]
    k = k.reshape_view({B, T, H, D}).transpose_view(1, 2); // [B, H, T, D]
    v = v.reshape_view({B, T, H, D}).transpose_view(1, 2); // [B, H, T, D]

    // Scaled dot-product attention
    nn::Tensor attn_scores = q.mm(k.transpose_view(2, 3)) / std::sqrt(static_cast<float>(D)); // [B, H, T, T]
